...
**📖 New to /araseo? See [QUICK_START.md](QUICK_START.md) for a 30-second tutorial!**
...
```
araseo/
├── SKILL.md                          # Main skill instructions (368 lines)
├── README.md                         # This file - comprehensive guide
├── QUICK_START.md                    # 30-second tutorial
├── templates/
│   ├── flowchart-schema.json         # JSON Graph Format schema (121 lines)
│   └── ui-mockup-schema.json         # UI component schema (91 lines)
├── examples/
│   ├── planning-docs/                # Input markdown examples
│   │   ├── simple-flowchart.md
│   │   ├── decision-flow.md
...
# Expected: JSON Graph Format with 3 nodes
```

**Quick Test (UI Mockup)**:
```bash
# Create test mockup
echo "# 테스트 페이지\n\nHeader: 로고 | 메뉴\nMain: 제목, 버튼\nFooter: © 2026" > test-ui.md
...
# Expected: UI mockup JSON with 3 sections
```

**Comprehensive Test**:
1. Use existing examples in `examples/planning-docs/`
2. Run `/araseo` on each example
3. Compare output with `examples/output/`
4. Verify JSON structure matches expected schema
5. Check Korean text is preserved correctly
...
**Core Functionality**:
- [x] SKILL.md has complete frontmatter (name, description, argument-hint, etc.)
- [x] JSON schemas are well-documented (flowchart, UI mockup)
- [x] Conversion process is clearly explained (step-by-step)
- [x] Error handling instructions are included

**Examples & Templates**:
- [x] Flowchart template exists (`templates/flowchart-schema.json`)
- [x] UI mockup template exists (`templates/ui-mockup-schema.json`)
- [x] At least 3 flowchart examples with input/output pairs
- [x] At least 2 UI mockup examples with input/output pairs
- [x] Examples cover common patterns (decision flows, forms, etc.)

**Documentation**:
- [x] README.md explains skill purpose and usage
- [x] Installation and usage instructions are clear
- [x] Examples are documented with descriptions
- [x] Troubleshooting section addresses common issues
- [x] Best practices are documented

**Compliance**:
- [x] Follows Claude Code Skills spec
- [x] Uses allowed-tools (Read, Write)
- [x] Output format rules are explicit (no code fences)
